#Model retriev from https://github.com/NimaVahdat/FoodSeg_mask2former
...
def load_model_and_processor(device):
    ...
    """
    directory_path = "checkpoints/mask2former_foodseg.pth"
...
def predict_masks(input_image_path):
    ...
    # === Nuevo ===
    # Agregamos info de segmentación para el fusionador
    seg_info = {
        "image_area": original_image_np.shape[0] * original_image_np.shape[1],
        "segments": [
            {
                "class_name": id2label.get(seg["label_id"], "unknown"),
                "mask_area": int((segmentation_mask == seg["id"]).sum())
            }
            for seg in segments_info
        ]
    }

    return output_result, seg_info
